...
void P_InitPicAnims(void)
{
  ...
  for (i = 0 ; animdefs[i].istexture != -1 ; i++)
  {
    ...
    lastanim->speed = SwapLong(
                        animdefs[i].speed); // killough 5/5/98: add SwapLong()
    lastanim++;
  }
...
boolean P_CanUnlockGenDoor(line_t* line, player_t* player)
{
  ...
  case RCard:
    ...
    {
      player->message = skulliscard ? s_PD_REDK :
  ...
  case BCard:
    ...
    {
      player->message = skulliscard ? s_PD_BLUEK :
  ...
  case YCard:
    ...
    {
      player->message = skulliscard ? s_PD_YELLOWK :
